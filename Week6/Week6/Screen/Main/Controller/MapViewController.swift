...
extension MapViewController {
     ...
     */
    // 설정까지 이동 || 설정 세부 화면까지 이동
    // 한번도 설정 앱에 들어가지 않았거나 막 다운 받은 앱이거나
...
extension MapViewController: CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateLocations locations: [CLLocation]) {
        ...
        // ex) 지도를 다시 세팅
        // 현재 사용자의 위치로 이동, title은 변하지 않음
        if let coordinate = locations.last?.coordinate {
            setRegionAndAnnotation(center: coordinate)
        }
        
        // 적당한 시점에 위치 업데이트 중지 
        locationManager.stopUpdatingLocation() // 어느정도 위치가 바뀌면 > accuarcy가 호출되어서 위치 변경 감지
    }
    ...
    // 거부 했다가 설정에서 변경 || notDetermined에서 허용을 했거나 등
    // 허용 했어서 위치를 가지고 오는 와중에 설정에서 거부하고 돌아온다면?
